...
impl AnalyticsPlugin {
    ...
    fn analyze_message(&mut self, message: &GuiChatMessage) {
        ...
        // ユニーク視聴者数更新
        *self.user_message_counts.entry(message.author.clone()).or_insert(0) += 1;
        ...
        *self.stats.hourly_distribution.entry(hour).or_insert(0) += 1;
        ...
        // 平均メッセージ長更新
        let current_total_length = self.stats.average_message_length * (self.stats.total_messages - 1) as f64;
        self.stats.average_message_length = (current_total_length + message.content.len() as f64) / self.stats.total_messages as f64;
        
        // スーパーチャット金額集計
        if let MessageType::SuperChat { amount } | MessageType::SuperSticker { amount } = &message.message_type {
    ...
    fn calculate_engagement_rate(&mut self) {
        ...
        let active_users = self.user_message_counts.values().filter(|&&count| count > 1).count();
        self.stats.engagement_rate = (active_users as f64 / self.stats.unique_viewers as f64) * 100.0;
    }
    
    /// ピーク時間を更新
    fn update_peak_hour(&mut self) {
        if let Some((peak_hour, _max_count)) = self.stats.hourly_distribution
            .iter()
    ...
    fn update_top_chatters(&mut self) {
        let mut chatters: Vec<(String, usize)> = self.user_message_counts
            .iter()
    ...
    async fn generate_report(&self) -> serde_json::Value {
        let session_duration = if let (Some(start), Some(end)) = (self.stats.first_message_time, self.stats.last_message_time) {
    ...
    pub async fn update_config(&mut self, config: AnalyticsConfig) -> LiscovResult<()> {
        ...
        if let Some(context) = &self.context {
            ...
            context.config_access.set_config(&context.plugin_id, "analytics_config", config_json).await?;
        }
...
impl Plugin for AnalyticsPlugin {
    ...
    async fn initialize(&mut self, context: PluginContext) -> LiscovResult<()> {
        // 保存された設定を読み込み
        if let Ok(Some(config_value)) = context.config_access.get_config(&context.plugin_id, "analytics_config").await {
        ...
        self.session_start_time = Some(chrono::Utc::now());
        context.logger.info(&context.plugin_id, "Analytics Plugin initialized");
    ...
    async fn shutdown(&mut self) -> LiscovResult<()> {
        if let Some(context) = &self.context {
            ...
            context.logger.info(&context.plugin_id, &format!("Final analytics report: {}", final_report));
            context.logger.info(&context.plugin_id, "Analytics Plugin shutting down");
        }
    ...
    async fn handle_event(&mut self, event: PluginEvent) -> LiscovResult<PluginResult> {
        match event {
            PluginEvent::ApplicationStarted => {
                self.reset_stats();
                if let Some(context) = &self.context {
                    context.logger.info(&context.plugin_id, "Analytics session started");
                }
            ...
            PluginEvent::MessageReceived(message) => {
                ...
                if self.config.verbose_logging {
                    if let Some(context) = &self.context {
                        context.logger.debug(&context.plugin_id, 
                            &format!("Analyzed message from {}: {} chars", message.author, message.content.len()));
                    }
                }
                ...
                if self.should_send_report() {
                    ...
                    if let Some(context) = &self.context {
                        context.event_sender.send_custom_event("analytics_report".to_string(), report.clone()).await?;
                        context.logger.info(&context.plugin_id, "Analytics report generated");
                    }
            ...
            PluginEvent::ConfigurationChanged { key, value } => {
                if key == "analytics_config" {
                    if let Ok(config) = serde_json::from_value::<AnalyticsConfig>(value) {
                        ...
                        if let Some(context) = &self.context {
                            context.logger.info(&context.plugin_id, "Analytics configuration updated");
                        }
                    }
                }
                Ok(PluginResult::Success)
            ...
            PluginEvent::ApplicationStopping => {
                ...
                if let Some(context) = &self.context {
                    context.event_sender.send_custom_event("final_analytics_report".to_string(), final_report.clone()).await?;
                    context.logger.info(&context.plugin_id, "Final analytics report sent");
                }
...
mod tests {
    ...
    fn test_superchat_stats() {
        ...
        let superchat = create_test_message("supporter", "Thank you!", 
