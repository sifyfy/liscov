...
| 要素 | サイズ | 色 | 備考 |
|------|--------|-----|------|
| タイムスタンプ | 10px固定 | `#64748b`（灰） | HH:MM:SS形式、表示/非表示切り替え可 |
...
|-----|-----|
| トリガー | 新着メッセージ到着 |
| 条件 | `auto_scroll_enabled` が true |
| 判定 | スクロール位置がボトムから100px以内で有効化 |

#### コントロール

| ボタン | 動作 |
|-------|------|
| 📍 最新に戻る | 自動スクロール有効化 + 即座にスクロール |
| 自動スクロールチェックボックス | ON/OFF切り替え |
