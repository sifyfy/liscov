...
| コンポーネント | 役割 |
|--------------|------|
| `InputSection.svelte` | URL入力、接続開始（常時表示、多接続対応） |
| `ConnectionList.svelte` | アクティブ接続リスト、個別/全切断ボタン |
...
| `streamTitle` | `string \| null` | 後方互換（最初の接続のタイトル） |
...
| メッセージ表示 | 色付き左ラインと配信者名ラベルで配信元を区別 |
